require 'rails_helper'

RSpec.describe WarehousesController, type: :controller do

  # This should return the minimal set of attributes required to create a valid
  # Warehouse. As you add validations to Warehouse, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # WarehousesController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET #index" do
    it "assigns all warehouses as @warehouses" do
      warehouse = Warehouse.create! valid_attributes
      get :index, params: {}, session: valid_session
      expect(assigns(:warehouses)).to eq([warehouse])
    end
  end

  describe "GET #show" do
    it "assigns the requested warehouse as @warehouse" do
      warehouse = Warehouse.create! valid_attributes
      get :show, params: {id: warehouse.to_param}, session: valid_session
      expect(assigns(:warehouse)).to eq(warehouse)
    end
  end

  describe "GET #new" do
    it "assigns a new warehouse as @warehouse" do
      get :new, params: {}, session: valid_session
      expect(assigns(:warehouse)).to be_a_new(Warehouse)
    end
  end

  describe "GET #edit" do
    it "assigns the requested warehouse as @warehouse" do
      warehouse = Warehouse.create! valid_attributes
      get :edit, params: {id: warehouse.to_param}, session: valid_session
      expect(assigns(:warehouse)).to eq(warehouse)
    end
  end

  describe "POST #create" do
    context "with valid params" do
      it "creates a new Warehouse" do
        expect {
          post :create, params: {warehouse: valid_attributes}, session: valid_session
        }.to change(Warehouse, :count).by(1)
      end

      it "assigns a newly created warehouse as @warehouse" do
        post :create, params: {warehouse: valid_attributes}, session: valid_session
        expect(assigns(:warehouse)).to be_a(Warehouse)
        expect(assigns(:warehouse)).to be_persisted
      end

      it "redirects to the created warehouse" do
        post :create, params: {warehouse: valid_attributes}, session: valid_session
        expect(response).to redirect_to(Warehouse.last)
      end
    end

    context "with invalid params" do
      it "assigns a newly created but unsaved warehouse as @warehouse" do
        post :create, params: {warehouse: invalid_attributes}, session: valid_session
        expect(assigns(:warehouse)).to be_a_new(Warehouse)
      end

      it "re-renders the 'new' template" do
        post :create, params: {warehouse: invalid_attributes}, session: valid_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested warehouse" do
        warehouse = Warehouse.create! valid_attributes
        put :update, params: {id: warehouse.to_param, warehouse: new_attributes}, session: valid_session
        warehouse.reload
        skip("Add assertions for updated state")
      end

      it "assigns the requested warehouse as @warehouse" do
        warehouse = Warehouse.create! valid_attributes
        put :update, params: {id: warehouse.to_param, warehouse: valid_attributes}, session: valid_session
        expect(assigns(:warehouse)).to eq(warehouse)
      end

      it "redirects to the warehouse" do
        warehouse = Warehouse.create! valid_attributes
        put :update, params: {id: warehouse.to_param, warehouse: valid_attributes}, session: valid_session
        expect(response).to redirect_to(warehouse)
      end
    end

    context "with invalid params" do
      it "assigns the warehouse as @warehouse" do
        warehouse = Warehouse.create! valid_attributes
        put :update, params: {id: warehouse.to_param, warehouse: invalid_attributes}, session: valid_session
        expect(assigns(:warehouse)).to eq(warehouse)
      end

      it "re-renders the 'edit' template" do
        warehouse = Warehouse.create! valid_attributes
        put :update, params: {id: warehouse.to_param, warehouse: invalid_attributes}, session: valid_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE #destroy" do
    it "destroys the requested warehouse" do
      warehouse = Warehouse.create! valid_attributes
      expect {
        delete :destroy, params: {id: warehouse.to_param}, session: valid_session
      }.to change(Warehouse, :count).by(-1)
    end

    it "redirects to the warehouses list" do
      warehouse = Warehouse.create! valid_attributes
      delete :destroy, params: {id: warehouse.to_param}, session: valid_session
      expect(response).to redirect_to(warehouses_url)
    end
  end

end
